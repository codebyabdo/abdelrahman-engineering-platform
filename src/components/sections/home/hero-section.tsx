...
export function HeroSection() {
  ...
  return (
    <section className="relative overflow-hidden px-4 sm:px-6">
      ...
      <div className="relative z-10 mx-auto max-w-5xl space-y-6 text-center sm:text-left sm:space-y-8">
        {/* Status Badge */}
        <motion.div
        ...
        >
          ...
          <p className="max-w-3xl pt-1 text-base font-light leading-relaxed text-white/60 sm:pt-2 sm:text-lg lg:text-xl">
            ...
            and polished digital experiences with{" "}
        ...
        >
          {/* Projects */}
          <Link href="/projects">
            <span className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2.5 text-[10px] font-bold uppercase tracking-widest text-white shadow-lg shadow-blue-600/30 transition-all hover:bg-blue-500 hover:shadow-blue-600/50 sm:px-6 sm:py-3.5 sm:text-xs">
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 sm:justify-start sm:gap-2">
            {PERSONAL_INFO.stack.map((item) => (
              <span
                key={item}
                className="rounded bg-white/3 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider text-white/70 sm:px-3 sm:py-1 sm:text-[11px]"
              >
                {item}
              </span>
            ))}
